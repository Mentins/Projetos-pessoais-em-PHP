    ...
    if(isset($_POST['email']) && isset($_POST['senha']) && !empty($_POST['email']) && !empty($_POST['senha'])){
        ...
        if($usuario){
            ...
            if($usuario['status'] == "confirmado"){
                //CRIAR UM TOKEN
                $token = sha1(uniqid().date('d/m/a-H:i:s'));
                ...
                $erro_login = "Confirme o cadastro no e-mail enviado!";
            }
...
<body>
    <form method="post">
        ...
        ?>
            <div class="sucesso animate__animated animate__backInLeft">
                    Usuário cadastrado com sucesso!!
            </div>
            <script>
                setTimeout(() => {
                    $('.sucesso').addClass('animate__backOutRight')
                }, 3000);
            </script>
        <?php } ?>
